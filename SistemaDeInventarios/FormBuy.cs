...
namespace SistemaDeInventarios
{
    public partial class FormBuy : Form
    {
                ...
                } else
                {
                    if (cProd.ReadProduct(tbProductID.Text, ref prod))
                    {
                        if (prod.sProductName != null)
                        {
                            ...
                            tbSalePrice.Text = prod.dBuyPrice.ToString();
                        } else
                        {
        ...
        private void FormBuy_Load(object sender, EventArgs e)
        {
            ...
            dgvBuy.Columns[0].Width = 60;
            dgvBuy.Columns[1].Width = 120;
            dgvBuy.Columns[2].Width = 60;
            dgvBuy.Columns[3].Width = 60;
            dgvBuy.Columns[4].Width = 60;
            dgvBuy.Columns[5].Width = 120;
            ...
            try
            {
                if (tbProductID.Text != "ID" && tbProduct.Text != "Nombre" && tbAmount.Text != "0.0" && cbWarehouse.Text != "Nombre")
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        dgvBuy.Rows.Add(tbProductID.Text, tbProduct.Text, tbAmount.Text, dt2.Rows[0][3].ToString(), (price * amount), dt.Rows[0][1].ToString());
                        Clear();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dgvBuy.Rows.Count > 0)
                {
                    ...
                    if (move.BuyProduct(lProdID, lQuantity, lPrice, lWareID, today))
                    {
                        MessageBox.Show("Entrada guardada");
                        Clear();
                        dgvBuy.Rows.Clear();
                    }
                    else
                ...
                {
                    MessageBox.Show("Por favor agregue un dato a la entrada");
                }
            } catch(Exception ex)
            {
